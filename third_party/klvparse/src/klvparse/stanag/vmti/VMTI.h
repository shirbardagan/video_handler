...
namespace klvparse {
    ...
    public:
        ...
        VMTI();
        VMTI(Context ctx);
        VMTI(std::weak_ptr<Context> ctx);
        VMTI(std::shared_ptr<Context> ctx);
        virtual ~VMTI();
    ...
    namespace parsers {
        bool vmti_local_set_parser(ByteReader &reader, nl::json *res, std::shared_ptr<Context> ctx);
    } // namespace parsers

    namespace encoders {
        bool vmti_local_set_encoder(const nl::json &item, std::vector<uint8_t> *res, std::shared_ptr<Context> ctx);
    } // namespace encoders
